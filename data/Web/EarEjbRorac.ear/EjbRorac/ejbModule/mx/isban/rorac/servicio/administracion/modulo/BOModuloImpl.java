...
public class BOModuloImpl extends Architech implements BOModulo {
	...
	public List<BeanModulo> obtenerModulosPorGrupo(
		...
		final BeanModuloRespuesta resultadoConsulta = daoModulo
				.obtenerModulosPorGrupos(sessionBean, grupos.split(REGEX_SPLIT));
	 ...
	 */
	private void establecerPantallas(final ArchitechSessionBean sessionBean,
			final List<BeanModulo> modulos, final String grupos)
			throws BusinessException {
		this.info("Iniciando la busqueda de todas las pantallas relacionadas a un usuario....");
		for (BeanModulo beanModulo : modulos) {
			beanModulo.setNombreModulo(UtileriasNegocio
					.normalizarTexto(beanModulo.getNombreModulo()));
			final BeanPantallaRespuesta respuesta = daoPantalla
					.obtenerPantallasPorGruposModulo(sessionBean,
							beanModulo.getIdModulo(), grupos.split(REGEX_SPLIT));
			verificarRespuesta(respuesta);
			final List<BeanPantalla> pantallas = respuesta.getPantallas();
			for (BeanPantalla beanPantalla : pantallas) {
				beanPantalla.setNombrePantalla(UtileriasNegocio
						.normalizarTexto(beanPantalla.getNombrePantalla()));
			}
